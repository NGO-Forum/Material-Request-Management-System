    ...
    <v-row>
      <v-col cols="12">
        ...
        <div class="d-flex justify-space-between align-center mb-6">
          <h1 class="text-h4 font-weight-bold">Categories Management</h1>
        ...
        <v-dialog v-model="deleteDialog" max-width="420">
          <v-card>
            ...
            <v-card-text>
              Delete <strong>{{ categoryToDelete?.name }}</strong>?<br>
              <small class="text-medium-emphasis">Cannot delete categories with materials assigned.</small>
            </v-card-text>
            <v-card-actions>
              <v-spacer />
...
const visibleHeaders = computed(() => {
  ...
  if (smAndDown.value) headers = headers.filter(h => h.key !== "description");
  if (xs.value) headers = headers.filter(h => h.key !== "created_at");
  return headers;
});
...
const loadCategories = async () => {
  try {
    ...
    categories.value = (res.data.data || res.data || []).map((c: any) => ({
      ...c,
...
const saveCategory = async () => {
  ...
  try {
    ...
    if (form.value.id) {
      const res = await axiosClient.put(`/categories/${form.value.id}`, payload);
      ...
      newCat.created_at = newCat.created_at || new Date().toISOString();
      categories.value.unshift(newCat);
      showMessage("Category created successfully");
    }
    closeDialog();
  } catch (err: any) {
    const msg = err.response?.data?.message || (err.response?.data?.errors ? Object.values(err.response.data.errors).flat().join(", ") : "Operation failed");
...
const deleteCategory = async () => {
  ...
  try {
    deleting.value = true;
    await axiosClient.delete(`/categories/${categoryToDelete.value.id}`);
    categories.value = categories.value.filter(c => c.id !== categoryToDelete.value!.id);
    showMessage("Category deleted successfully");
  } catch (err: any) {
    showMessage(err.response?.data?.message || "Cannot delete category (in use)", "error");
  } finally {
